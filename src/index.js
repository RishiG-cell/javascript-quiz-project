document.addEventListener("DOMContentLoaded", () => {
  ...
  function showQuestion() {
    ...
    progressBar.style.width =
      (100 / quiz.questions.length) * quiz.currentQuestionIndex + "%"; // This value is hardcoded as a placeholder
    ...
    quiz.questions[quiz.currentQuestionIndex].choices.forEach((choice) => {
      ...
      newChoice.innerHTML = `<input type="radio" name="choice" value="${choice}">
          <label>${choice}</label>
        <br>`;
  ...
  function nextButtonHandler() {
    ...
    allChoices.forEach((oneChoice) => {
      if (oneChoice.checked) {
        checked = oneChoice.value;
      }
    });

    const correctAnswer = quiz.checkAnswer(checked);
  ...
  function showResults() {
    ...
    resultContainer.innerText = `You scored ${correctAnswers} out of ${quiz.questions.length} correct answers!`; // This value is hardcoded as a placeholder
  }
});
